...
impl<'a> SuiClient<'a> {
    ...
    pub(crate) fn upgrade(&self, package_dir: &Path) -> anyhow::Result<PublishOutput> {
        let output =
            self.cli
                .run_binary_command("sui", &["client", "upgrade", "--json"], package_dir)?;
        serde_json::from_slice(&output.stdout).context("failed to parse upgrade output")
    }
}
