...
const GENESIS_WORK_DIR: &str = "/tmp/sui/genesis";
...
pub(crate) async fn run(ctx: &Ctx, config_path: &Path) -> anyhow::Result<()> {
    ...
    if !is_genesis_present(&s3, &config).await? {
        let yaml = render_genesis_yaml(&config, &config.validator_keys, &config.sponsor_key)?;
        let yaml_path = PathBuf::from(GENESIS_WORK_DIR).join(GENESIS_OUTPUT_FILE);
        let work_path = PathBuf::from(GENESIS_WORK_DIR).join("work");
    ...
    ctx.k8s
        ...
        .store_put(config.output_name, BTreeMap::new(), BTreeMap::new())
        .await?;
...
async fn is_genesis_present(s3: &S3Client, config: &GenesisConfig) -> anyhow::Result<bool> {
    match s3
        .head_object()
        .bucket(&config.s3.bucket)
        .key(GENESIS_BLOB)
