...
use crate::reconcilers::key_node::KeyArchiveComponent;
...
#[component(name = "workload-archive")]
pub(crate) struct WorkloadArchiveComponent;

#[derive(Clone, Serialize)]
pub(crate) struct WorkloadArchiveReconciler {
    pub(crate) operator: OperatorSettings,
}

#[async_trait::async_trait]
impl Reconciler for WorkloadArchiveReconciler {
    ...
    async fn reconcile(&self, client: &K8sClient, network: &EchBoardNetwork) -> Result<NodeState> {
        ...
        let instance_name = WorkloadArchiveComponent.instance_name(&network_name)?;
        let mut labels = WorkloadArchiveComponent.labels(&network_name)?;
                ...
                BTreeMap::from([(
                    ...
                    yamls::fullnode::render_archive(
                        ...
                        &serde_json::from_str(
                            &client
                                .namespaced::<Secret>(&namespace)
                                .store_load(KeyArchiveComponent.instance_name(&network_name)?)
                                .await?
                                .get(KEYS)?,
                        )?,
                        &serde_json::from_str(
        ...
        let pod_spec = SuiNodePodBuilder {
            ...
            component_name: "archive-fullnode".into(),
