...
pub(crate) fn render_read(
    ...
    render(
        ...
        seed_peers,
        FullnodeMode {
            service_name: WorkloadFullnodeComponent { ordinal }.name(&owner),
...
pub(crate) fn render_archive(
    ...
    render(
        ...
        seed_peers,
        FullnodeMode {
            service_name: WorkloadArchiveComponent.name(&owner),
            writes_archive: true,
        },
        s3_access_key,
        s3_secret_key,
...
fn render(
    ...
    let fullnode_config = FullnodeYamlConfig {
        ...
        state_archive_read_config: Some(vec![StateArchiveReadConfig {
            ingestion_url: format!("s3://{}", network.spec.archive.bucket),
            concurrency: 5,
            remote_store_options: vec![
                ("aws_endpoint".into(), network.spec.archive.endpoint.clone()),
                ("aws_access_key_id".into(), s3_access_key.to_string()),
                ("aws_secret_access_key".into(), s3_secret_key.to_string()),
            ],
        }]),
        state_snapshot_write_config: mode.writes_archive.then(|| StateSnapshotWriteConfig {
            object_store_config: ObjectStoreConfig {
                object_store: "S3".into(),
                bucket: network.spec.archive.bucket.clone(),
                aws_endpoint: network.spec.archive.endpoint.clone(),
